...
def handle_add(request_json):
	...
	if do_async and len(spt_children) > 0:
		executor = concurrent.futures.ThreadPoolExecutor(max_workers=len(spt_children))
	for neighbour in spt_children:
		logging.debug("{}:JOB ID {}:HANDLE ADD:NEIGHBOURS:neighbours = {}".format(dt.now(), request_json['job_id'], neighbour))
		if do_async:
...
def handle_remove(request_json):
	...
	for neighbour in neighbours:
		logging.debug("{}:JOB ID {}:HANDLE REMOVE:NEIGHBOURS:neighbours = {}, send_json = {}".format(dt.now(), request_json['job_id'], neighbour, send_json))
		if do_async:
			executor.submit(load_url, generate_url(neighbour), send_json)
		else:
			my_session.post(generate_url(neighbour), json=send_json, timeout=HTTP_TIMEOUT)
...
def handle_del(request_json):
	...
	if tasks[RequestType.DELETE.name]:
		...
		for neighbour in tasks[RequestType.DELETE.name]["neighbours"]:
			logging.debug("{}:JOB ID {}:HANDLE DEL:NEIGHBOURS:{}:{}".format(dt.now(), request_json['job_id'], neighbour, send_json))
			if do_async:
				executor.submit(load_url, generate_url(neighbour), send_json)
			else:
				my_session.post(generate_url(neighbour), json=send_json, timeout=HTTP_TIMEOUT)
...
def handle_control(request_json):
	if request_json[SUBTYPE] == RequestSubtype.INIT.name:
		...
		logging.debug("{}:JOB ID {}:HANDLE CONTROL:FILE:hash = {}".format(dt.now(), request_json['job_id'], request_json['file_hash']))
		return 0
...
def do_new_query(request_json):
	...
	if removed_hash != 0:
		...
		logging.debug("{}:JOB ID {}:TRIGGER REMOVE:removed_hash = {}".format(dt.now(), request_json['job_id'], removed_hash))
...
def do_download(fhash, ip, request_json):
	...
	logging.debug(
		"{}:JOB ID {}:HANDLE JOB:DOWNLOAD:file_hash = {}, location = {}, source_ip = {}".format(dt.now(), request_json['job_id'], fhash, ip, table.src_ips[str(fhash)]['source']))
