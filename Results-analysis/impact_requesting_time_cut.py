...
for xp in cache_sizes:
    sizes = sorted(list(dict_download_time[trace_name].keys()))
    to_plot = []
    for size in sizes:
        aaa = comparison["new-{}".format(xp)][size]["download"] + comparison["new-{}".format(xp)][size]["req"]
        dht = comparison["dht-{}".format(xp)][size]["download"] + comparison["dht-{}".format(xp)][size]["req"]
        reduction = (float(dht - aaa)/float(dht)) * 100
        to_plot.append(reduction)
    plt.plot(x_labels, to_plot, label="Cache size: {}".format(xp))
axes = plt.gca()
axes.yaxis.grid()
plt.legend()
