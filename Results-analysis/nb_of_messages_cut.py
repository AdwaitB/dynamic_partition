...
import datetime
from cons import get_stop_time
...
for cache_size in cache_sizes:
    stop_time.update({"new-{}".format(cache_size):  get_stop_time('new', cache_size)})
    stop_time.update({"dht-{}".format(cache_size):  get_stop_time('dht', cache_size)})
...
for filename in list_of_xps:
    ...
    for node in os.listdir(os.getcwd()):
        ...
        if os.path.exists("_node_stderr.txt"):
            nbr_messages = 0
            cmd = """ grep "Nb of JSON" _node_stderr.txt """
            nbr_messages_list = os.popen(cmd).read()
            nbr_messages_list_splitted = nbr_messages_list.split('\n')
            del nbr_messages_list_splitted[-1]
            for entry in nbr_messages_list_splitted:
                time_of_entry = datetime.fromisoformat(entry.split("root")[1].split("Nb")[0][1:len(entry)-1][:-2])
                if time_of_entry < stop_time[filename]:
                    nbr_messages += 1
                #else: break

            if filename.startswith("dht"):  # We need to add the request messages to get the localization.
                nbr_messages_req = 0
                cmd = """ grep "HANDLE DHT:REQUEST" _node_stderr.txt """
                nbr_messages_req_list = os.popen(cmd).read()
                nbr_messages_req_list_splitted = nbr_messages_req_list.split('\n')
                del nbr_messages_req_list_splitted[-1]
                for entry in nbr_messages_req_list_splitted:
            ...
            for entry in sent_msg_splitted:
                time_of_entry = datetime.fromisoformat(entry.split("root")[1].split("send")[0][1:len(entry) - 1][:-2])
                if time_of_entry < stop_time[filename]:
...
for size in cache_sizes:
    ...
    list_of_nodes = [x for x in dict_nb_messages[filename].keys()]
    x_label = list(range(1, len(dict_nb_messages[filename]) + 1, 1))
    y_dht = [len(dict_nb_messages["dht-{}".format(size)][x]['sent_msg']) + dict_nb_messages["dht-{}".format(size)][x][
