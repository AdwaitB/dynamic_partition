		...
		if os.path.isdir(filename):
			...
			if cache_size not in already_done:
				if filename.startswith("new") or filename.startswith("dht"):
					...
					for node in os.listdir(os.getcwd()):
						...
						if os.path.exists("_node_stderr.txt"):
							...
							for entry in sources_splitted:
								...
								dict_of_download_sources[filename][job_id].update({filename: (node, file_hash, location, source)})

					already_done.add(filename.split("-")[1])
					if "new" in filename: filename = filename.replace("new", "dht")
					elif "dht" in filename: filename = filename.replace("dht", "new")
					if filename not in dict_of_download_sources: dict_of_download_sources.update({filename: {}})
					os.chdir(path + "/" + filename + "/")
					for node in os.listdir(os.getcwd()):
						os.chdir(path + "/" + filename+"/"+node+ "/root/deploy/traces")
					...
					going_to_source = {xp_1: 0, xp_2: 0}
					for id in dict_of_download_sources[xp_1]:
						if id in dict_of_download_sources[xp_1] and id in dict_of_download_sources[xp_2]:
							if dict_of_download_sources[xp_1][id][xp_1][2] != dict_of_download_sources[xp_1][id][xp_1][3]:
								not_going_to_source[xp_1] += 1
							else:
								going_to_source[xp_1] += 1
							if dict_of_download_sources[xp_2][id][xp_2][2] != dict_of_download_sources[xp_2][id][xp_2][3]:
								not_going_to_source[xp_2] += 1
							else:
								going_to_source[xp_2] += 1

							if dict_of_download_sources[xp_1][id][xp_1][2] != dict_of_download_sources[xp_2][id][xp_2][2]:
								not_going_same_destination += 1
						else:
							difference_processed_jobs += 1

					going_to_source[xp_1] = round(float(going_to_source[xp_1] * 100) / float(NB_JOBS), 1)
					going_to_source[xp_2] = round(float(going_to_source[xp_2]) * 100 / float(NB_JOBS), 1)
					not_going_to_source[xp_1] = round(float(not_going_to_source[xp_1] *100)/ float(NB_JOBS), 1)
					not_going_to_source[xp_2] = round(float(not_going_to_source[xp_2]) *100/ float(NB_JOBS), 1)
					results[filename].update({'not_going_same_destination': round(float(not_going_same_destination)*100/float(NB_JOBS), 1),'not_going_to_source': not_going_to_source, 'going_to_source': going_to_source})
					os.chdir(path)
...
os.chdir(path)
for filename in dict_of_download_sources:
	print('{} - locally_served: {}'.format(filename, round(float((NB_JOBS - len(dict_of_download_sources[filename].keys())) *100)/float(NB_JOBS), 1)))
